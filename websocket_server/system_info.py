...
def get_all():
    message = {"system_info": {"CPU Temp (motor plate)": get_cpu_temperature(),
                           "SD (motor plate)": get_disk_usage(),
                           "RAM (motor plate)": get_ram_usage(),
                           "CPU (motor plate)": get_cpu_usage(),
                           "Uptime (motor plate)": get_uptime(),
                           "Last reboot (motor plate)": get_last_reboot(),
                           "IP (motor plate)": get_ip_address()}}
